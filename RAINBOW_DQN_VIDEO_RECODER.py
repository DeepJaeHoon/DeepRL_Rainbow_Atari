...
print(device)
summary = SummaryWriter("C:/Users/dlwog/Desktop/RL/tensorboard/rainbow/rainbow_video")
...
class NoisyLinear(nn.Module):
    ...
    def forward(self, input):
        ...
        if self.training:
            return F.linear(input, self.weight_mu, self.bias_mu)#F.linear(input, self.weight_mu + self.weight_sigma * self.weight_epsilon, self.bias_mu + self.bias_sigma * self.bias_epsilon)
        else:
...
def main():
    ...
    env = gym.make("ALE/Breakout-v5",render_mode = "rgb_array_list")#, render_mode = "human") # 강화학습의 ENV 생성
    ...
    rainbow = torch.load("C:/Users/dlwog/Desktop/RL/history/" + "last_carnival_rainbow_period.pt", map_location=device)
    ...
    for n_epi in range(0, episode): # 에피소드 진행
        ...
        if score >= 425:
            trigger = lambda step_index: step_starting_index <= step_index <= step
            print("SAVE Video")
            save_video(
                env.render(),
                f"C:/Users/dlwog/Desktop/rq/" + "rainbow.mp4",
